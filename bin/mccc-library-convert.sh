...
convert_to_mysql_old()
{
	python access-dump.py $LIBRARY_MDB LIB \
	| sed '/CREATE TABLE .*/ {N; s/CREATE TABLE .*\n (/&\n\t`id` int(10) NOT NULL auto_increment,\n \t PRIMARY KEY  (`id`), /g}' \
	| mysql --default-character-set=utf8 --user=root --password=password mymccc
}

convert_to_mysql()
{
	python access-dump.py $LIBRARY_MDB MCCC_Library \
	| sed '/CREATE TABLE .*/ {N; s/CREATE TABLE .*\n (/&\n\t`id` int(10) NOT NULL auto_increment,\n \t PRIMARY KEY  (`id`), /g}' >mccc-library.sql
